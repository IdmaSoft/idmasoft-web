...
8. **La posición inicial del cubo (`computeInitialNudge`) no debe depender de un alto de texto hardcodeado por breakpoint — el sitio es multi-idioma.** El español envuelve a más líneas que el inglés para el mismo contenido (títulos/botones más largos), así que un `textBottom` fijo por breakpoint (afinado solo contra el inglés) hace que el cubo termine demasiado arriba — pegado o detrás de los botones — apenas se traduce. Fix: `HeroContent.tsx` mide el borde inferior real del bloque de texto (`getBoundingClientRect()`, restando el `y` del scroll para leer la posición de reposo) vía `ResizeObserver`, lo reporta a `HeroSection` → `HeroCore` → `Cube`, y `computeInitialNudge` lo usa en vez de la constante `INITIAL_GAP_TUNING` (que ahora es solo el *fallback* para el primer frame, antes de medir — nunca el valor final). El cubo sigue oculto (`opacity 0`) hasta que `useIsHydrated() && textBottom !== null`, para no mostrar la posición del fallback y después saltar. Los textos de las caras del cubo (`hero.cube.<id>.title/description` en `messages/*/home.json`) también deben mantenerse en español de largo igual o menor al inglés — las tarjetas ya están al límite de espacio (ver gotcha 4), y una traducción más larga puede volver a cortar contenido.
